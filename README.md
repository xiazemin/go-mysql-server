...
```
go get github.com/xiazemin/go-mysql-server@latest
```
...
- [Not
  threadsafe](https://github.com/xiazemin/go-mysql-server/issues/1306). To
  avoid concurrency issues, limit DDL and DML statements (`CREATE
  TABLE`, `INSERT`, etc.) to a single goroutine.
- [No transaction
  support](https://github.com/xiazemin/go-mysql-server/issues/1506). Statements
  like `START TRANSACTION`, `ROLLBACK`, and `COMMIT` are no-ops.
- [Non-performant index
  implementation](https://github.com/xiazemin/go-mysql-server/issues/1347). Indexed
